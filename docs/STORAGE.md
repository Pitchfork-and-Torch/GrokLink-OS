# Storage - GrokLink OS v3.9 Field Card (host/CI 3.9.1)
...
| `glkfs` | File-backed card image (`glk_storage_init_card`) | v3.9 remount + CRC/append tests |
| `absent` | No card / SD probe fail (`glk_storage_init_device`) | ROM-passive |

`storage_status.probe` is honest:

| probe | Meaning |
|-------|---------|
| `posix` | Host directory backend |
| `host_image` | File-backed GLKFS card image |
| `host_sim` | Device probe on a host build (no SPI SD) |
| `no_card` | Device: no card / no SPI ACK |
| `no_spi` | Device: bus timeout |
| `present` | Device: card ACK (not claimed in host CI) |
| `none` | Shutdown / not probed |
...
Physical cards are **never auto-formatted**. Empty host images may be formatted. Table CRC failure stays `corrupt`. File CRC is checked on read.

GLKFS append is streaming (not a 2 KiB RAM cap). Rewrites reuse the existing extent when the new size fits. Old extents are not compacted.
