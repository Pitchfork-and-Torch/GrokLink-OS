...
## Current state (2026-09-17) - v3.9.1 host/CI (device DFU not recut)
...
| SD FatFs / card hot-load | GLKFS host/CI (3.9). Device probe absent without card ACK. No SPI CMD0 yet. |
| BLE M0+ stack | `ble_status` skeleton only (3.9). Not GATT, not a phone app. |
...
| SDMMC / SPI SD on device | GLKFS bind + SD probe (absent without card). Host cook: CRC-on-read, streaming append, no auto-format of corrupt/physical cards. Device SPI CMD0/CMD8 still human-gated. | **Done (3.9 host proof; device probe honest-absent)** |
| Hot-load skill packages from card | signed optional (FNV + host Ed25519) | **Done (3.8/3.9)** |
| Autonomous offline loop without PC | agent task on device + power | Partial (3.8 vault + 3.9 card) |
...
| BLE status | M0+ IPCC **skeleton** (3.9 `ble_status`). Not GATT. |
...
| **B - Native OS** | `GrokLink-OS-v3.8.0-radio.dfu` | Primary research OS + LLM observe + missions |
