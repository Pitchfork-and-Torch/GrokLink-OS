...
1. Board SPI glue in `platform/stm32wb55/hal/glk_board_spi_r.c` (bit-bang SPI_R) OK  
2. Build: `.\tools\build_dfu.ps1 -Profile OsRadio` -> `GrokLink-OS-v3.8.0-radio.dfu`  
3. DFU entry (lab): unplug -> hold **BACK+OK ~30 s** -> plug USB  
4. Probe: `{"cmd":"subghz_probe"}` -> VERSIONâ‰ˆ**20** (`0x14`) on real CC1101 OK  
