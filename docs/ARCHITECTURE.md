...
Multi-band spectrum: **planner** issues sequential jobs with **minimum settle**
(`GLK_SPECTRUM_SETTLE_MS`), **duty budget** window, SubGHz **arbiter**, and
global **circuit breaker** - never a tight multi-band loop on the RPC thread;
RPC/agent cannot bypass these limits.

---

## 8. Storage (v3.8)

- SD card primary; **host FS shim** is first-class (POSIX/Windows). Device littlefs/SDMMC binds the same `glk_storage_*` API.
- Logical root: `/groklink` - config, missions, skills, logs, blacklist, vault, state, healthcare.
- Crash-safe helpers: atomic write (temp + rename), integrity markers, degraded modes (absent/corrupt/full).
- Journaled vault: `vault/events.jsonl` + `vault/chain.meta`; audit remains append-only hash-chained.
- Crash-safe mission state: `state/mission_<id>.resume`.
- Prefer SD for bulk skills/missions/vault over internal flash. See [STORAGE.md](STORAGE.md).
