...
int main(int argc, char** argv) {
    ...
    if (glk_storage_init(g_sd) == GLK_OK) {
        ...
        glk_policy_reload_profile(&g_policy, g_sd);
        (void)glk_skill_scan_storage();
        /* also accept absolute path scan for host sim */
        char skills[560];
...
#endif
        printf("[boot] storage mode=%s skills=%u vault_persist=%s\n",
               glk_storage_mode_str(glk_storage_mode()),
               (unsigned)glk_skill_count(),
               glk_vault_persistent() ? "yes" : "no");
    } else {
        glk_agent_init(&g_agent, &g_policy);
        glk_audit_init("audit_host.jsonl");
        printf("[boot] SD root missing - degraded mode + ROM catalog (passive only)\n");
    }
