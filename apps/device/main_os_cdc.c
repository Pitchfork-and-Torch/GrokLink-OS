...
static int is_light_cmd(const char* line) {
    ...
    return strstr(line, "ping") != 0 || strstr(line, "status") != 0 || strstr(line, "help") != 0 ||
           ...
           strstr(line, "reboot_dfu") != 0 || strstr(line, "storage_status") != 0 ||
           strstr(line, "ble_status") != 0;
}
...
int main(void) {
    ...
    for (int i = 0; i < 200; i++) usbd_poll(&udev);
    glk_ble_init();
    for (int i = 0; i < 50; i++) usbd_poll(&udev);
    (void)glk_storage_init_device(); /* absent without card - ROM passive */
    for (int i = 0; i < 50; i++) usbd_poll(&udev);
    glk_policy_init(&s_pol);
    glk_policy_set_global(&s_pol);
    glk_policy_set_sd_present(&s_pol, glk_storage_present());
