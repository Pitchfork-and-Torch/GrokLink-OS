...
## 3.9.1 - Host/CI honesty (2026-09-17)

Public cut is **host/CI honesty**. This tag does **not** recut DFU. No over-the-air claims.

Device silicon stays at the last human flash. Last packaged OsRadio in `dist/dfu` is **v3.8.0**. v3.9.0 named a `v3.9.0-radio.dfu` path but did not attach one. Field Card on silicon needs a human `build_dfu.ps1` then flash.

- **GLKFS:** streaming append (no 2048-byte cap), rewrite reuses an extent when it fits, file CRC checked on read, table CRC fail stays **corrupt** (does not reformat).
- **Honest SD probe:** `storage_status.probe` is `posix` | `host_image` | `host_sim` | `no_card` | `no_spi` | `present`. Host `init_device` returns `NOSUPPORT` (`host_sim`). Device still absent without a card ACK. Never a fake RAM disk. Physical cards are never auto-formatted.
- **BLE skeleton:** `tx`, `advertising`, `ipcc` are false. `glk_ble_tick` is a no-op.
- **PC bridge:** skip USB banners, first complete JSON object only. Bench connect fails closed without hanging a TUI.
- **Lab codec tests:** truncated / CRC mismatch / invalid hex / oversize message. GLK1 only.

## 3.9.0 - Field Card (2026-09-05)

The card is the SD. The bench is a tiny Desktop. BLE is a status whisper.

- **GLKFS** over 512-byte blocks: format, mount, remount-surviving files. Host file image is the CI proof.
- Device **SD probe** compiled in. No card / no SPI ACK stays **absent** (ROM-passive). Does not fake a RAM disk.
- `storage_status` JSON adds `backend` (`posix` | `glkfs` | `absent`).
- **Ed25519** `GLKSIG1-ED25519` format on host/CI (pure Python verify). Device still uses GLKSIG1 FNV; `GLK_FEATURE_SIGNED_SKILLS=1` fail-closes unsigned packs. Not secure-boot.
- **BLE skeleton** RPC `ble_status` (`state: skeleton`, `phone_app: false`). No GATT product.
- **Bench Desktop** `groklink-os bench` / `tools/start_bench.ps1`. Observe-only. Not Grok Orbit.
- **USB soak** `tools/usb_soak.ps1` (under 8s, SKIP if no device).
- **CI** `.github/workflows/host-tests.yml` (cmake ctest + pytest). Device DFU stays human-gated.
- GrokRPC API **7**.
