...
static uint8_t connect(bool connect) {
    /*
     * Skip Battery Charging Detection (BCD).
     * Upstream BCD path toggles BCDR without required settle delays; on some
     * hosts that leaves D+/D- in a half-defined state and Windows reports
     * "Device Descriptor Request Failed" (VID_0000). Pull-up only is enough
     * for CDC ACM on a standard downstream port.
     */
    USB->BCDR = (connect) ? USB_BCDR_DPPU : 0;
    return connect ? usbd_lane_sdp : usbd_lane_dsc;
}
