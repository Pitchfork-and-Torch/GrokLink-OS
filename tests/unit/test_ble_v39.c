...
int main(void) {
    ...
    assert(strstr(js, "\"gatt\":false") != NULL);
    assert(strstr(js, "\"tx\":false") != NULL);
    assert(strstr(js, "\"advertising\":false") != NULL);
    assert(strstr(js, "\"ipcc\":false") != NULL);
    glk_ble_tick();
    assert(glk_ble_state() == GLK_BLE_SKELETON);
