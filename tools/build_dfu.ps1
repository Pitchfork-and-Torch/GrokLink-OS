...
function Add-OsCoreSources {
  ...
  # v3.9 Field Card: GLKFS + SD probe (absent without card) + BLE skeleton
  $script:sources += @{ Path = (Join-Path $Root "services\src\storage\glk_storage.c"); Lang = "c" }
  $script:sources += @{ Path = (Join-Path $Root "services\src\storage\glk_fs.c"); Lang = "c" }
  $script:sources += @{ Path = (Join-Path $Root "platform\stm32wb55\hal\glk_board_sd.c"); Lang = "c" }
  $script:sources += @{ Path = (Join-Path $Root "services\src\rpc\glk_ble.c"); Lang = "c" }
...
if ($Profile -eq "Bringup") {
  ...
  $outBase = "GrokLink-OS-v3.9.1-radio"
  $label = "GrokLink OS 3.9.1 host/CI (human-gated DFU)"
  $version = "3.9.1-radio"
  ...
  $outBase = "GrokLink-OS-v3.9.1-rpc"
  $label = "GrokLink OS 3.9.1 RPC (human-gated DFU)"
  $version = "3.9.1-rpc"
  $commonFlags += $deviceBudget
}
