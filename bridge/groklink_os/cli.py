...
def skill_status_cmd() -> None:
    """Loaded skill registry status (count, signed enforce, ids)."""
    c = _client()
    try:
        rprint(c.skill_status())
    finally:
        c.close()


@app.command("skill-scan")
def skill_scan_cmd() -> None:
    """Rescan SD skills/ (hot-load). ROM catalog remains."""
    c = _client()
    try:
        rprint(c.skill_scan())
    finally:
...
@app.command("skill-unload")
def skill_unload_cmd(skill_id: str = typer.Argument(..., help="SD skill id to unload")) -> None:
    """Hot-unload an SD skill (ROM skills are protected)."""
    c = _client()
    try:
        rprint(c.skill_unload(skill_id))
    finally:
        c.close()
...
def storage_status_cmd() -> None:
    """SD / storage mode, backend, layout (v3.9 Field Card)."""
    c = _client()
    try:
        rprint(c.storage_status())
    finally:
        c.close()
...
def vault_status_cmd() -> None:
    """Vault count, persistent flag, sealed, chain root."""
    c = _client()
    try:
        rprint(c.vault_status())
    finally:
        c.close()


@app.command("vault-flush")
def vault_flush_cmd() -> None:
    """Flush vault chain meta to storage."""
    c = _client()
    try:
        rprint(c.vault_flush())
    finally:
        c.close()


@app.command("spectrum-status")
def spectrum_status_cmd() -> None:
    """Spectrum planner duty budget, settle, breaker trips."""
    c = _client()
    try:
        rprint(c.spectrum_status())
    finally:
        c.close()


@app.command("power-status")
def power_status_cmd() -> None:
    """Power mode + battery estimate (host sim supports GLK_BATTERY_PCT)."""
    c = _client()
    try:
        rprint(c.power_status())
    finally:
        c.close()


@app.command("ble-status")
def ble_status_cmd() -> None:
    """BLE skeleton status (not a phone app)."""
    c = _client()
    try:
        rprint(c.ble_status())
    finally:
...
@app.command("bench")
def bench_cmd() -> None:
    """Open Bench Desktop (tk). Do not launch from a Grok TUI job."""
    from groklink_os.bench import main as bench_main
