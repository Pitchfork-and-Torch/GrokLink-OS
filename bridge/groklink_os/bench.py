...
DFU_HINT = (
    "Human-gated DFU: do not flash from a Grok TUI job. 3.9.1 does not recut DFU.\n"
    "  Last packaged OsRadio: dist\\dfu\\GrokLink-OS-v3.8.0-radio.dfu\n"
    "  Field Card on silicon needs a human build+flash (no OTA).\n"
...
def bench_status(serial_port: str | None = None, timeout: float = 3.0) -> dict[str, Any]:
    """Connect, edu-ack, status, storage, ble. Observe-only. Never TX.

    Fail-closed: do not auto-grab a COM handle (exclusive CDC hangs a TUI).
    No serial_port -> TCP host-sim only.
    """
    out: dict[str, Any] = {"version": __version__, "edu": EDU_ACK, "tx": False, "ok": True}
    try:
        if serial_port:
            c = open_client(serial_port=serial_port, timeout=timeout, prefer="serial")
        else:
            c = open_client(prefer="tcp", timeout=timeout, host="127.0.0.1")
    except (OSError, ConnectionError, TimeoutError) as e:
        out["ok"] = False
        out["error"] = str(e)
        out["note"] = "No device / host-sim. Bench does not TX and does not flash."
        return out
    try:
...
def main() -> None:
    ...
    root = tk.Tk()
    root.title("GrokLink Bench 3.9.1 host/CI")
