...
def test_decode_truncated_and_crc() -> None:
    b = LabBeacon(lab_id=7, counter=11, message="CRC")
    raw = encode_beacon(b)
    trunc = decode_beacon_bytes(raw[:8])
    assert trunc["ok"] is False
    assert trunc["error"] in ("truncated", "not_lab_beacon")
    bad = bytearray(raw)
    bad[-1] ^= 0xFF
    crc = decode_beacon_bytes(bytes(bad))
    assert crc["ok"] is False
    assert crc["error"] == "crc_mismatch"
    extra = decode_beacon_bytes(raw + b"\x00\x01")
    assert extra["ok"] is True
    assert extra["beacon"]["message"] == "CRC"


def test_decode_invalid_hex_and_oversize() -> None:
    bad = decode_beacon_hex("zz not hex")
    assert bad["ok"] is False
    long_msg = "X" * 64
    try:
        encode_beacon(LabBeacon(lab_id=1, counter=1, message=long_msg))
