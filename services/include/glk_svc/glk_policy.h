...
/**
 * Policy gates for storage / skill load / vault (v3.8).
 * Actuators remain default-deny; these are informational + soft deny on corrupt.
 */
typedef enum {
    GLK_POL_OP_STORAGE_WRITE = 1,
    GLK_POL_OP_SKILL_LOAD = 2,
    GLK_POL_OP_SKILL_UNLOAD = 3,
    GLK_POL_OP_VAULT_CLEAR = 4,
    GLK_POL_OP_VAULT_SEAL = 5,
    GLK_POL_OP_MISSION_RESUME = 6,
} glk_policy_storage_op_t;
...
/** Mark storage corrupt -> elevated actions fail closed (TX already denied). */
