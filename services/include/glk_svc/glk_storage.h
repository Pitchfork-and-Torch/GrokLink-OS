...
typedef enum {
    GLK_STOR_BE_NONE = 0,
    GLK_STOR_BE_POSIX = 1, /* host directory */
    GLK_STOR_BE_GLKFS = 2, /* Field Card image / device SD */
} glk_storage_backend_t;

const char* glk_storage_backend_str(glk_storage_backend_t b);
glk_storage_backend_t glk_storage_backend(void);

/**
 * Last probe reason (honest, never a fake RAM disk):
 * posix | host_image | host_sim | no_card | no_spi | present | none
 */
const char* glk_storage_probe_str(void);

/** Init with absolute or relative host path to the groklink root (POSIX). */
glk_err_t glk_storage_init(const char* root_path);

/** Init Field Card: file-backed GLKFS image (host). Formats if empty. */
glk_err_t glk_storage_init_card(const char* image_path);

/** Device: probe SD. Absent if no card. Does not fake RAM. */
