...
glk_err_t glk_rpc_handle_json(glk_rpc_t* rpc, const char* req_json, char* resp, size_t resp_len) {
    ...
    if (strcmp(cmd, "status") == 0) {
        ...
        glk_kernel_stats(&ks);
        char stor[192];
        glk_storage_status_json(stor, sizeof(stor));
        snprintf(
            resp,
            resp_len,
            "{\"ok\":true,\"version\":\"%s\",\"api\":%d,\"edu\":%s,\"sd\":%s,"
            "\"storage_mode\":\"%s\",\"blacklist_ok\":%s,\"radio\":%d,\"heap_free\":%u,"
            "\"missions\":%u,\"skills\":%u,\"vault\":%u,\"vault_persistent\":%s,"
            "\"profile\":\"%s\",\"medsec_strict\":%s,\"not_medical_device\":true,"
            "\"power\":\"%s\",\"disclaimer\":\"NOT a medical device. Authorized research only.\"}",
            GLK_VERSION_STRING,
            GLK_RPC_API,
            glk_policy_edu_acked(rpc->policy) ? "true" : "false",
            rpc->policy->sd_present ? "true" : "false",
            glk_storage_mode_str(glk_storage_mode()),
            ...
            glk_power_mode_str(glk_power_mode()));
        (void)stor;
        return GLK_OK;
    }

    if (strcmp(cmd, "storage_status") == 0) {
        char st[320];
        glk_storage_status_json(st, sizeof(st));
        snprintf(resp, resp_len, "{\"ok\":true,\"storage\":%s}", st);
        return GLK_OK;
    }

    if (strcmp(cmd, "ble_status") == 0) {
        char st[240];
        glk_ble_status_json(st, sizeof(st));
        snprintf(resp, resp_len, "{\"ok\":true,\"ble\":%s}", st);
        return GLK_OK;
    }

    if (strcmp(cmd, "skill_status") == 0) {
        char st[400];
        glk_skill_status_json(st, sizeof(st));
        snprintf(resp, resp_len, "{\"ok\":true,\"skill\":%s}", st);
        return GLK_OK;
    }

    if (strcmp(cmd, "skill_scan") == 0) {
        glk_policy_decision_t d =
            glk_policy_check_storage_op(rpc->policy, GLK_POL_OP_SKILL_LOAD, GLK_ACTOR_RPC);
        if (d.result == GLK_POLICY_DENY) {
            snprintf(resp, resp_len, "{\"ok\":false,\"error\":\"%s\"}", d.reason);
            return GLK_OK;
        }
        glk_err_t e = glk_skill_scan_storage();
        if (e == GLK_ERR_DEGRADED || e == GLK_ERR_NOTFOUND) {
            /* try explicit path under sd_root */
            if (rpc->sd_root[0]) {
                char path[560];
                snprintf(path, sizeof(path), "%s/skills", rpc->sd_root);
                e = glk_skill_scan(path);
            }
        }
        char list[400];
    ...
    if (strcmp(cmd, "skill_unload") == 0) {
        ...
        glk_err_t e = glk_skill_unload(id);
        snprintf(resp, resp_len, "{\"ok\":%s,\"err\":%d,\"id\":\"%s\"}",
                 e == GLK_OK ? "true" : "false", (int)e, id);
        return GLK_OK;
    }

    if (strcmp(cmd, "vault_status") == 0) {
        char st[192];
        glk_vault_status_json(st, sizeof(st));
        snprintf(resp, resp_len, "{\"ok\":true,\"vault\":%s}", st);
        return GLK_OK;
    }

    if (strcmp(cmd, "vault_flush") == 0) {
        glk_err_t e = glk_vault_flush();
        snprintf(resp, resp_len, "{\"ok\":%s,\"err\":%d}", e == GLK_OK ? "true" : "false", (int)e);
        return GLK_OK;
    }

    if (strcmp(cmd, "vault_seal") == 0) {
        glk_policy_decision_t d =
            glk_policy_check_storage_op(rpc->policy, GLK_POL_OP_VAULT_SEAL, GLK_ACTOR_RPC);
        ...
        char secret[64];
        secret[0] = 0;
        json_get_str(req_json, "secret", secret, sizeof(secret));
        glk_err_t e = glk_vault_seal(secret);
        snprintf(resp, resp_len, "{\"ok\":%s,\"sealed\":%s}",
                 e == GLK_OK ? "true" : "false",
                 glk_vault_sealed() ? "true" : "false");
        return GLK_OK;
    }

    if (strcmp(cmd, "vault_unseal") == 0) {
        char secret[64];
        secret[0] = 0;
        json_get_str(req_json, "secret", secret, sizeof(secret));
        glk_err_t e = glk_vault_unseal(secret);
        snprintf(resp, resp_len, "{\"ok\":%s,\"sealed\":%s}",
                 e == GLK_OK ? "true" : "false",
                 glk_vault_sealed() ? "true" : "false");
        return GLK_OK;
    ...
    if (strcmp(cmd, "spectrum_status") == 0 || strcmp(cmd, "planner_status") == 0) {
        char st[256];
        glk_radio_planner_status_json(st, sizeof(st));
        snprintf(resp, resp_len, "{\"ok\":true,\"planner\":%s}", st);
        return GLK_OK;
    }

    if (strcmp(cmd, "power_status") == 0) {
        char st[128];
        glk_power_status_json(st, sizeof(st));
        snprintf(resp, resp_len, "{\"ok\":true,\"power\":%s}", st);
        return GLK_OK;
    }

    if (strcmp(cmd, "resume_save") == 0) {
        if (!rpc->agent) {
        ...
        id[0] = 0;
        json_get_str(req_json, "id", id, sizeof(id));
        if (!id[0] && rpc->agent->active_id[0]) strncpy(id, rpc->agent->active_id, sizeof(id) - 1);
        glk_err_t e = glk_agent_save_resume(rpc->agent, id);
        snprintf(resp, resp_len, "{\"ok\":%s,\"err\":%d,\"id\":\"%s\"}",
                 e == GLK_OK ? "true" : "false", (int)e, id);
        return GLK_OK;
    }

    if (strcmp(cmd, "resume_load") == 0) {
        if (!rpc->agent) {
            snprintf(resp, resp_len, "{\"ok\":false,\"error\":\"no_agent\"}");
            return GLK_OK;
        }
        ...
        id[0] = 0;
        json_get_str(req_json, "id", id, sizeof(id));
        glk_err_t e = glk_agent_load_resume(rpc->agent, id);
        snprintf(resp, resp_len, "{\"ok\":%s,\"err\":%d,\"id\":\"%s\"}",
                 e == GLK_OK ? "true" : "false", (int)e, id);
        return GLK_OK;
    }
    ...
    if (strcmp(cmd, "vault_clear") == 0) {
        glk_policy_decision_t d =
            glk_policy_check_storage_op(rpc->policy, GLK_POL_OP_VAULT_CLEAR, GLK_ACTOR_RPC);
        if (d.result != GLK_POLICY_ALLOW) {
            snprintf(resp, resp_len, "{\"ok\":false,\"error\":\"%s\"}", d.reason);
            return GLK_OK;
        }
        glk_err_t e = glk_vault_clear_persistent();
        snprintf(resp, resp_len, "{\"ok\":%s,\"cleared\":%s,\"err\":%d}",
                 e == GLK_OK ? "true" : "false",
                 e == GLK_OK ? "true" : "false",
                 (int)e);
        return GLK_OK;
