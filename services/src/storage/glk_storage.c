...
const char* glk_storage_backend_str(glk_storage_backend_t b) {
    switch (b) {
    case GLK_STOR_BE_POSIX: return "posix";
    case GLK_STOR_BE_GLKFS: return "glkfs";
    case GLK_STOR_BE_NONE:
    default: return "absent";
    }
}

glk_storage_backend_t glk_storage_backend(void) {
    return s_backend;
}

const char* glk_storage_probe_str(void) {
    return s_probe[0] ? s_probe : "none";
}
...
glk_err_t glk_storage_init(const char* root_path) {
    ...
    s_write_errors = 0;
    s_layout_ok = 0;
    s_backend = GLK_STOR_BE_POSIX;
    strncpy(s_probe, "posix", sizeof(s_probe) - 1);
...
glk_err_t glk_storage_init_card(const char* image_path) {
    if (!image_path || !image_path[0]) return GLK_ERR_INVAL;
    glk_storage_shutdown();
    strncpy(s_root, image_path, sizeof(s_root) - 1);
    s_write_errors = 0;
    s_layout_ok = 0;
    s_backend = GLK_STOR_BE_GLKFS;
    strncpy(s_probe, "host_image", sizeof(s_probe) - 1);
    const uint32_t blocks = 256; /* 128 KiB host card image */
    glk_err_t e = glk_blockdev_file_open(&s_card, image_path, blocks, false);
    if (e != GLK_OK) {
        e = glk_blockdev_file_open(&s_card, image_path, blocks, true);
        if (e != GLK_OK) {
            s_present = false;
            s_mode = GLK_STOR_ABSENT;
            s_backend = GLK_STOR_BE_NONE;
            strncpy(s_probe, "none", sizeof(s_probe) - 1);
            return e;
        }
        if (glk_fs_format(&s_card) != GLK_OK) {
            s_present = false;
            s_mode = GLK_STOR_ABSENT;
            return GLK_ERR_GENERIC;
        }
    } else {
        glk_err_t m = glk_fs_mount(&s_card);
        if (m == GLK_ERR_EMPTY) {
            if (glk_fs_format(&s_card) != GLK_OK) {
                s_present = false;
                s_mode = GLK_STOR_ABSENT;
            ...
            /* Honest: do not reformat a corrupt card image. */
            s_present = true;
...
glk_err_t glk_storage_init_device(void) {
    glk_storage_shutdown();
    strncpy(s_root, "/groklink", sizeof(s_root) - 1);
    s_backend = GLK_STOR_BE_GLKFS;
    glk_err_t e = glk_blockdev_sd_probe(&s_card);
    if (e != GLK_OK || !s_card.present) {
        s_present = false;
        s_mode = GLK_STOR_ABSENT;
        s_backend = GLK_STOR_BE_NONE;
        if (e == GLK_ERR_NOSUPPORT) strncpy(s_probe, "host_sim", sizeof(s_probe) - 1);
        else if (e == GLK_ERR_TIMEOUT) strncpy(s_probe, "no_spi", sizeof(s_probe) - 1);
        else strncpy(s_probe, "no_card", sizeof(s_probe) - 1);
        return e;
    }
    strncpy(s_probe, "present", sizeof(s_probe) - 1);
    glk_err_t m = glk_fs_mount(&s_card);
    if (m != GLK_OK) {
        /* Never auto-format a physical card. Empty/corrupt stays fail-closed. */
        s_present = true;
        s_mode = (m == GLK_ERR_EMPTY) ? GLK_STOR_DEGRADED : GLK_STOR_CORRUPT;
        s_layout_ok = 0;
        return (m == GLK_ERR_EMPTY) ? GLK_ERR_EMPTY : GLK_ERR_CORRUPT;
    }
    s_present = true;
    s_mode = GLK_STOR_DEGRADED;
    if (glk_storage_ensure_layout() == GLK_OK) {
...
void glk_storage_shutdown(void) {
    if (s_backend == GLK_STOR_BE_GLKFS) {
        glk_fs_unmount();
        glk_blockdev_file_close(&s_card);
    }
    s_backend = GLK_STOR_BE_NONE;
    s_present = false;
    s_mode = GLK_STOR_ABSENT;
...
glk_err_t glk_storage_ensure_layout(void) {
    if (s_backend == GLK_STOR_BE_GLKFS) {
        if (!glk_fs_mounted()) return GLK_ERR_NOTFOUND;
        /* Files appear on write. Marker proves the card is writable. */
        const char* probe = ".layout";
        const char okm[] = "ok\n";
        if (glk_fs_write(probe, okm, 3) != GLK_OK) return GLK_ERR_DEGRADED;
        s_layout_ok = 1;
        return GLK_OK;
    }
...
glk_err_t glk_storage_write_file(const char* rel, const void* data, size_t len) {
    ...
    if (s_backend == GLK_STOR_BE_GLKFS) {
        glk_err_t e = glk_fs_write(rel, data, len);
        if (e != GLK_OK) s_write_errors++;
        return e;
    }
    if (s_mode == GLK_STOR_CORRUPT) return GLK_ERR_CORRUPT;
...
glk_err_t glk_storage_append_file(const char* rel, const void* data, size_t len) {
    ...
    if (!glk_storage_usable()) return GLK_ERR_DEGRADED;
    if (s_backend == GLK_STOR_BE_GLKFS) {
        glk_err_t e = glk_fs_append(rel, data, len);
        if (e != GLK_OK) s_write_errors++;
        return e;
    }
    if (s_mode == GLK_STOR_CORRUPT) return GLK_ERR_CORRUPT;
...
glk_err_t glk_storage_read_file(const char* rel, void* data, size_t cap, size_t* out_len) {
    char path[512];
    if (!s_present) return GLK_ERR_NOTFOUND;
    if (s_backend == GLK_STOR_BE_GLKFS) return glk_fs_read(rel, data, cap, out_len);
    if (glk_storage_path(rel, path, sizeof(path)) != GLK_OK) return GLK_ERR_INVAL;
...
bool glk_storage_exists(const char* rel) {
    char path[512];
    if (s_backend == GLK_STOR_BE_GLKFS) return glk_fs_exists(rel);
...
glk_err_t glk_storage_remove(const char* rel) {
    char path[512];
    if (!glk_storage_usable()) return GLK_ERR_DEGRADED;
    if (s_backend == GLK_STOR_BE_GLKFS) return glk_fs_remove(rel);
    if (glk_storage_path(rel, path, sizeof(path)) != GLK_OK) return GLK_ERR_INVAL;
...
glk_err_t glk_storage_file_hash32(const char* rel, uint32_t* out_hash) {
    ...
    if (s_backend == GLK_STOR_BE_GLKFS) {
        return glk_fs_fnv32(rel, out_hash);
    }
    char path[512];
    if (glk_storage_path(rel, path, sizeof(path)) != GLK_OK) return GLK_ERR_INVAL;
    FILE* f = fopen(path, "rb");
...
size_t glk_storage_status_json(char* buf, size_t buflen) {
    ...
    char root_show[72];
    memset(root_show, 0, sizeof(root_show));
    strncpy(root_show, s_root, sizeof(root_show) - 1);
    uint32_t files = 0, used = 0;
    if (s_backend == GLK_STOR_BE_GLKFS && glk_fs_mounted()) {
        glk_fs_info(&files, &used);
    }
    return (size_t)snprintf(
        buf,
        buflen,
        "{\"present\":%s,\"mode\":\"%s\",\"usable\":%s,\"layout_ok\":%s,"
        "\"backend\":\"%s\",\"probe\":\"%s\",\"files\":%u,\"used_blocks\":%u,"
        "\"write_errors\":%u,\"root\":\"%s\"}",
        s_present ? "true" : "false",
        glk_storage_mode_str(s_mode),
        glk_storage_usable() ? "true" : "false",
        s_layout_ok ? "true" : "false",
        glk_storage_backend_str(s_backend),
        glk_storage_probe_str(),
        (unsigned)files,
        (unsigned)used,
