...
void glk_policy_set_storage_corrupt(glk_policy_state_t* st, bool corrupt) {
    if (!st) return;
    if (corrupt) {
        st->degraded = true;
        st->blacklist_ok = false; /* fail closed for elevated when media untrustworthy */
    }
}

glk_policy_decision_t glk_policy_check_storage_op(
    glk_policy_state_t* st,
    glk_policy_storage_op_t op,
    glk_actor_t actor) {
    glk_policy_decision_t d;
    memset(&d, 0, sizeof(d));
    if (!st || !st->initialized) {
        d.result = GLK_POLICY_DENY;
        strncpy(d.reason, "policy_uninit", sizeof(d.reason) - 1);
        return d;
    }
    /* Vault clear / seal need edu ack */
    if (op == GLK_POL_OP_VAULT_CLEAR || op == GLK_POL_OP_VAULT_SEAL) {
        if (!st->edu_acked) {
            d.result = GLK_POLICY_DENY;
            strncpy(d.reason, "edu_required", sizeof(d.reason) - 1);
            glk_audit_log(actor, "vault_op", GLK_RISK_SYSTEM, d.result, d.reason, NULL);
            return d;
        }
    }
    /* Writes / skill load require usable non-corrupt storage or ROM path */
    if (op == GLK_POL_OP_STORAGE_WRITE || op == GLK_POL_OP_SKILL_LOAD ||
        op == GLK_POL_OP_MISSION_RESUME) {
        if (!st->sd_present) {
            d.result = GLK_POLICY_DEGRADED;
            strncpy(d.reason, "no_sd_rom_only", sizeof(d.reason) - 1);
            glk_audit_log(actor, "storage_op", GLK_RISK_INFO, d.result, d.reason, NULL);
            return d;
        }
        if (st->degraded && !st->blacklist_ok && op == GLK_POL_OP_SKILL_LOAD) {
            /* still allow passive skill scan when degraded; elevated risk elsewhere */
        }
    }
    if (op == GLK_POL_OP_SKILL_UNLOAD) {
        /* always allow unload of SD skills */
    }
    d.result = GLK_POLICY_ALLOW;
    strncpy(d.reason, "allow", sizeof(d.reason) - 1);
    return d;
}
