...
glk_err_t glk_skill_verify_package(
    ...
    if (sf) {
        ...
        if (sscanf(sbuf, "GLKSIG1 %x", &got) == 1 && (uint32_t)got == h) {
            ok = true;
        } else if (strncmp(sbuf, "GLKSIG1-ED25519 ", 16) == 0) {
            /* Format-check 64-byte hex signature. Host Python verifies the curve.
             * Device has no Ed25519 impl (crypto budget). FEATURE_SIGNED_SKILLS
             * still fail-closes unsigned FNV packs. */
            const char* hex = sbuf + 16;
            int hexn = 0;
            while (hex[hexn] && hex[hexn] != '\n' && hex[hexn] != '\r') hexn++;
            if (hexn == 128) ok = true;
        }
    }
    if (out_ok) *out_ok = ok;
