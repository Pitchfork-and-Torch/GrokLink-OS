...
glk_err_t glk_agent_init(glk_agent_t* ag, glk_policy_state_t* policy) {
    ...
    if (glk_storage_usable()) {
        (void)glk_vault_bind_storage("vault/events.jsonl");
    }
    return GLK_OK;
}

bool glk_agent_should_defer_autonomy(const glk_agent_t* ag) {
    (void)ag;
    glk_power_mode_t m = glk_power_mode();
    if (m == GLK_PWR_DEEP_SLEEP) return true;
    int bat = glk_power_battery_pct();
    if (bat >= 0 && bat < 15) return true; /* battery-aware deferral */
    return false;
}

glk_err_t glk_agent_save_resume(const glk_agent_t* ag, const char* id) {
    if (!ag || !id || !id[0]) return GLK_ERR_INVAL;
    if (!glk_storage_usable()) return GLK_ERR_DEGRADED;
    const glk_mission_t* m = glk_agent_get(ag, id);
    if (!m) return GLK_ERR_NOTFOUND;
    char rel[96];
    snprintf(rel, sizeof(rel), "state/mission_%s.resume", id);
    char body[160];
    int n = snprintf(
        body,
        sizeof(body),
        "GLKRES1 id=%s pc=%u state=%u pulses=%ld\n",
        m->id,
        (unsigned)m->pc,
        (unsigned)m->state,
        (long)m->last_pulses);
    if (n <= 0) return GLK_ERR_GENERIC;
    return glk_storage_write_file(rel, body, (size_t)n);
}

glk_err_t glk_agent_load_resume(glk_agent_t* ag, const char* id) {
    if (!ag || !id || !id[0]) return GLK_ERR_INVAL;
    if (!glk_storage_usable()) return GLK_ERR_DEGRADED;
    ...
    char body[160];
    size_t n = 0;
    if (glk_storage_read_file(rel, body, sizeof(body) - 1, &n) != GLK_OK) return GLK_ERR_NOTFOUND;
    body[n] = 0;
    unsigned pc = 0, st = 0;
    long pulses = 0;
    char mid[GLK_MISSION_ID_MAX];
    mid[0] = 0;
    if (sscanf(body, "GLKRES1 id=%31s pc=%u state=%u pulses=%ld", mid, &pc, &st, &pulses) < 3) {
        return GLK_ERR_CORRUPT;
    }
    if (strcmp(mid, id) != 0) return GLK_ERR_CORRUPT;
    if (pc < m->step_count) m->pc = (uint16_t)pc;
    m->last_pulses = (int32_t)pulses;
    if (st == (unsigned)GLK_MISSION_RUNNING || st == (unsigned)GLK_MISSION_ARMED) {
...
glk_err_t glk_agent_tick(glk_agent_t* ag) {
    if (!ag) return GLK_ERR_INVAL;
    ag->ticks++;
    if (glk_agent_should_defer_autonomy(ag)) {
        return GLK_OK; /* power-aware: skip autonomy this tick */
    }
    if (!ag->active_id[0]) {
        ...
        for (size_t i = 0; i < ag->mission_count; i++) {
            if (ag->missions[i].autonomous &&
                ...
                (void)glk_agent_save_resume(ag, ag->missions[i].id);
                break;
            }
        }
    }
    ...
    if (m && m->state == GLK_MISSION_DONE && m->autonomous) {
        (void)glk_agent_save_resume(ag, m->id);
    ...
    if (m && (m->state == GLK_MISSION_ARMED || m->state == GLK_MISSION_RUNNING)) {
        glk_err_t e = glk_agent_run_once(ag);
        if ((ag->ticks & 7u) == 0u) (void)glk_agent_save_resume(ag, m->id);
        return e;
    }
    return GLK_OK;
}
